...
The current source registers **152 DUnit tests across eight classes**, including 17 fluent lifecycle characterization tests. The latest supplied DUnit GUI Test Runner evidence, dated **2026-09-19**, executed **152/152 tests**, with **0 failures**, **0 errors**, and **0 overrides**, matching the current source count and covering the lifecycle class. The earlier **135/135** Delphi 12 Community Edition/Windows 32-bit build-and-run evidence is retained only as historical validation. Current Method Toxicity CSV exports also cover both production `src` and `NewTests`: the highest measured `Toxicity` is **0.588** in the framework and **0.350** in `NewTests`; within `Rick.SQL.Tests.Fluent.Lifecycle.pas`, the maximum is **0.279**. Details and evidence boundaries are centralized under [`docs/testes`](docs/testes/README.md) and [Toxicity control](docs/engenharia/CONTROLE_DE_TOXICIDADE.md).
