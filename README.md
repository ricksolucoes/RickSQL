...
The official suite for new refactorings and behavioral fixes lives under `NewTests/` and uses DUnit with the GUI Test Runner. Exception-normalization tests are under `NewTests/src/Error/`, SQL parameter validation/identification coverage under `NewTests/src/Validation/`, transaction coverage under `NewTests/src/Transaction/`, FireDAC provider verification under `NewTests/src/Infrastructure/`, canonical `VendorLib` application coverage under `NewTests/src/ClientLibrary/`, provider/definition reuse coverage under `NewTests/src/Driver/`, and fluent lifecycle characterization under `NewTests/src/Facade/`. The `tests/` tree remains the legacy suite and reference material; `NewTests/` does not structurally depend on it.

The current source registers **158 DUnit tests across nine classes**, including 17 fluent lifecycle characterization tests and 6 materialization/`FetchAll` characterization tests. The latest supplied DUnit GUI Test Runner evidence, finished at **2026-09-20 10:13:44**, records **158 tests**, all with `PASS` results, **0 failures**, **0 errors**, and a **100% success rate**. The **152/152** run from 2026-09-19 and the earlier **135/135** Delphi 12 Community Edition/Windows 32-bit build-and-run evidence are retained only as historical validation. The Method Toxicity CSV files supplied in the 2026-09-20 validation round cover both production `src` and `NewTests`: the highest measured `Toxicity` is **0.588** in the framework and **0.350** in `NewTests`; in the new `Rick.SQL.Tests.DataSet.Materializer.pas` unit, the maximum is **0.217**. Details and evidence boundaries are centralized under [`docs/testes`](docs/testes/README.md) and [Toxicity control](docs/engenharia/CONTROLE_DE_TOXICIDADE.md).
