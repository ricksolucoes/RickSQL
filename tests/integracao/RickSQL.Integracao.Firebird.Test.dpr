program RickSQLIntegracaoFirebirdTest;
...
begin
  Result := TRickSQLIntegrationTestHelper.HasEnv('RICKSQL_FIREBIRD_DATABASE');
end;

function CriarConexao: TRickSQLConnectionOptions;
begin
  Result := TRickSQL.ConnectionOptions(TRickSQLDatabaseEngine.Firebird);
  Result.Server := TRickSQLIntegrationTestHelper.Env('RICKSQL_FIREBIRD_SERVER');
  Result.Database := TRickSQLIntegrationTestHelper.Env('RICKSQL_FIREBIRD_DATABASE');
  Result.UserName := TRickSQLIntegrationTestHelper.Env('RICKSQL_FIREBIRD_USERNAME');
  Result.Password := TRickSQLIntegrationTestHelper.Env('RICKSQL_FIREBIRD_PASSWORD');
  Result.ClientLibraryPath := TRickSQLIntegrationTestHelper.Env('RICKSQL_FIREBIRD_CLIENT_LIBRARY');
end;
...
procedure ExecutarIgnorandoErro(const AConnection: TRickSQLConnectionOptions;
  const ASQL: string);
begin
  TRickSQL.Execute(TRickSQL.Command(AConnection, ASQL));
end;

procedure PrepararBase(const AConnection: TRickSQLConnectionOptions);
begin
  ExecutarIgnorandoErro(AConnection, 'drop table ricksql_integracao');
  ExecutarSQL(AConnection, 'create table ricksql_integracao (' +
    'id integer not null primary key, nome varchar(80), ' +
    'valor numeric(15,2), data_venda timestamp, ativo smallint, ' +
    'observacao blob sub_type 0, nulo varchar(20))');
end;
...
begin
  ...
  LCommand.AddParameter(TRickSQLParameter.Create('NOME', 'Venda Firebird'));
  LCommand.AddParameter(TRickSQLParameter.Create('VALOR', 10.50));
  ...
  TRickSQLIntegrationTestHelper.CheckResult(LResult, 'Falha no insert Firebird.');
end;
...
begin
  ...
  try
    TRickSQLIntegrationTestHelper.CheckError(LError, 'Falha na consulta Firebird.');
...
begin
  ...
  begin
    Writeln('Firebird ignorado. Configure RICKSQL_FIREBIRD_DATABASE para executar.');
    Exit;
  end;
...
begin
  try
    ExecutarTestes;
    Writeln('Testes de integração Firebird finalizados.');
  except
    on E: Exception do
    begin
      Writeln('Falha nos testes de integração Firebird: ', E.Message);
      Halt(1);
    end;
  end;
end.
