...
A fonte atual registra **152 testes DUnit em oito classes**, incluindo 17 testes de caracterização do lifecycle fluent. A evidência mais recente fornecida do DUnit GUI Test Runner, datada de **19/09/2026**, executou **152/152 testes**, com **0 falhas**, **0 erros** e **0 overrides**, correspondendo à contagem da fonte atual e incluindo a classe de lifecycle. A evidência anterior de build e execução **135/135** em Delphi 12 Community Edition/Windows 32-bit permanece somente como histórico. Os CSVs atuais de Method Toxicity também cobrem `src` e `NewTests`: o maior `Toxicity` medido é **0,588** no framework e **0,350** em `NewTests`; em `Rick.SQL.Tests.Fluent.Lifecycle.pas`, o máximo é **0,279**. Os detalhes e limites estão em [`docs/testes`](docs/testes/README.pt-BR.md) e em [Controle de toxicidade](docs/engenharia/CONTROLE_DE_TOXICIDADE.pt-BR.md).
