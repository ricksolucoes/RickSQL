...
A suíte oficial para novas refatorações e correções comportamentais fica em `NewTests/` e usa DUnit com GUI Test Runner. As units de normalização de exceptions estão em `NewTests/src/Error/`, a validação/identificação de parâmetros SQL em `NewTests/src/Validation/`, a cobertura transacional em `NewTests/src/Transaction/`, a verificação do provider FireDAC em `NewTests/src/Infrastructure/`, a cobertura da aplicação canônica de `VendorLib` em `NewTests/src/ClientLibrary/`, a cobertura de reutilização de provider/definition em `NewTests/src/Driver/` e a caracterização do lifecycle fluent em `NewTests/src/Facade/`. A árvore `tests/` permanece como suíte legada e material auxiliar; não é dependência estrutural de `NewTests/`.

A fonte atual registra **158 testes DUnit em nove classes**, incluindo 17 testes de caracterização do lifecycle fluent e 6 testes de caracterização da materialização/`FetchAll`. A evidência mais recente fornecida do DUnit GUI Test Runner, finalizada em **20/09/2026 10:13:44**, registra **158 testes**, todos com resultado `PASS`, **0 falhas**, **0 erros** e **100% de sucesso**. A rodada **152/152** de 19/09/2026 e a evidência anterior de build e execução **135/135** em Delphi 12 Community Edition/Windows 32-bit permanecem somente como histórico. Os CSVs de Method Toxicity fornecidos na rodada de 20/09/2026 cobrem `src` e `NewTests`: o maior `Toxicity` medido é **0,588** no framework e **0,350** em `NewTests`; na nova `Rick.SQL.Tests.DataSet.Materializer.pas`, o máximo é **0,217**. Os detalhes e limites estão em [`docs/testes`](docs/testes/README.pt-BR.md) e em [Controle de toxicidade](docs/engenharia/CONTROLE_DE_TOXICIDADE.pt-BR.md).
